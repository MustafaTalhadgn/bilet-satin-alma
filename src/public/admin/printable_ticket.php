...
if ($admin_role === 'user') {
    die("Bu sayfaya erişim yetkiniz yok. Lütfen 'Biletlerim' sayfasındaki indirme linkini kullanın.");
}
...
try {
    ...
    if ($admin_role === 'company') {

        $comp_stmt = $pdo->prepare("SELECT company_id FROM User WHERE id = :admin_id");
        $comp_stmt->execute([':admin_id' => $admin_id]);
...
<head>
    ...
    <style>
        ...
        .ticket-container { max-width: 800px; margin: 50px auto; background-color: #fff; border: 1px solid #ddd; box-shadow: 0 0 10px rgba(0,0,0,0.1); }
        .ticket-header { background-color: #f1f1f1; padding: 20px; border-bottom: 1px solid #ddd; }
        .ticket-body { padding: 30px; }
        .ticket-body h4 { border-bottom: 2px solid #eee; padding-bottom: 10px; margin-bottom: 20px; }
        .print-button-container { text-align: center; margin-top: 20px; }
        @media print {
            body { background-color: #fff; }
            .print-button-container, .navbar { display: none !important; }
            .ticket-container { margin: 0; border: none; box-shadow: none; }
        }
    </style>
</head>
<body>
...
<div class="ticket-container">
    ...
    <div class="ticket-body">
        ...
        <p><strong>E-posta:</strong> <?php echo htmlspecialchars($ticket_data['user_email']); ?></p>
    ...
    <div class="ticket-footer border-top p-3 text-center text-muted">
        Bu bilet, yönetici panelinden oluşturulmuştur.
    </div>
</div>
